  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        let auth;
        if (this.isLogin) {
          response = await axios.post(`https://safebites.onrender.com/api/v1/authentications`, authData);
          const user= await axios.get(`https://safebites.onrender.com/api/v1/users/${response.data.id}`, {
        ...
        localStorage.setItem('user', JSON.stringify({
          ...
          response = await axios.post(`https://safebites.onrender.com/api/v1/users`, authData);
          auth = await axios.post(`https://safebites.onrender.com/api/v1/authentications`, {
            email: this.email,
            password: this.password
          });
