...
export default {
  ...
  try {
    const userResponse = await axios.post(`https://safebites.onrender.com/api/v1/users`, {
  ...
  .filter(range => range.length === 2 && range.every(n => !isNaN(n)));
    ...
    const authenticate = await axios.post(`https://safebites.onrender.com/api/v1/authentications`, {
    ...
    const restaurantResponse = await axios.post(`https://safebites.onrender.com/api/v1/restaurants`, {
